...
// Sección Inversión & Energía — el vertical corporativo/energético de Bochile
// (Vaca Muerta, parques industriales, inversión). Reutiliza la estética de
// Empresas (.emp) para mantener coherencia. El CTA va directo a Karina (vacamuerta).
export default function InversionEnergia() {
  const { t } = useLang()
  const e = t.inversion
  return (
    <section className="section wrap" id="inversion">
      <div className="emp">
        <div className="emp-left">
          <div className="sec-kicker reveal">{e.kicker}</div>
          <h2 className="sec-title reveal">{e.title1} {e.title2}</h2>
          <p className="emp-sub reveal">{e.sub}</p>
          <a className="emp-cta reveal" data-cursor target="_blank" rel="noopener" href={waTo('vacamuerta', e.waMsg)}>
            <WaIcon /> {e.cta}
          </a>
        </div>
